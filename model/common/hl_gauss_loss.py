...
class HLGaussLoss(nn.Module): 
    def __init__(self, min_value: float, max_value: float, num_bins: int, sigma: float): 
        ...
        self.sigma = sigma 
        # Use register_buffer so it moves to device automatically
    ...
    def transform_to_probs(self, target: torch.Tensor) -> torch.Tensor: 
        cdf_evals = torch.special.erf( 
            ...
            / (torch.sqrt(torch.tensor(2.0, device=target.device)) * self.sigma) 
        ) 
        z = cdf_evals[..., -1] - cdf_evals[..., 0] 
        bin_probs = cdf_evals[..., 1:] - cdf_evals[..., :-1] 
        return bin_probs / (z.unsqueeze(-1) + 1e-8) # Added epsilon for stability
